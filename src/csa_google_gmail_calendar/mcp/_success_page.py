"""The HTML shown in the browser after a successful `login` consent.

Ported from `../csa-google-workspace/src/csa_google_workspace/mcp/_success_page.py`, text
changed to name this server; the CSA branding, colours and logo are unchanged (brand rules
forbid recoloring the asset - it carries the superseded orange (#F98526); the current palette
value is #FF7A00, which is why this page uses no orange of its own).

Served by the loopback listener that receives Google's redirect (`_auth_flow.py`). Self-contained
apart from the CSA TypeKit stylesheet: the page must still render correctly offline or if
TypeKit is blocked, so every font has a real fallback stack.
"""
